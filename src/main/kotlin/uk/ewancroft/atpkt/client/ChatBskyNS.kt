...
class ChatBskyGroupNS(private val agent: Agent) {
    ...
    suspend fun getJoinLinkPreviews(params: GetJoinLinkPreviewsInput): Result<GetJoinLinkPreviewsOutput> = runCatching {
        agent.xrpc.executeQuery("chat.bsky.group.getJoinLinkPreviews", params).getOrThrow()
    }
    suspend fun listJoinRequests(params: ListJoinRequestsInput): Result<ListJoinRequestsOutput> = runCatching {
        agent.xrpc.executeQuery("chat.bsky.group.listJoinRequests", params).getOrThrow()
    }
